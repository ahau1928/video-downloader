...
#[serde(default)]
pub struct Settings {pub subtitles:crate::subtitles::Options,pub transcode:crate::transcode::Options,pub output_dir:String,pub proxy:String,pub use_proxy:bool,pub cookies_browser:String,pub quality_height:Option<u64>,pub output_mode:String,pub quality_mode:String,pub keep_source:bool,pub concurrency:usize,pub auto_update:bool,pub batch_output_dir:String,pub filename_suffix:bool,pub filename_codecs:bool}
impl Default for Settings {fn default()->Self{Self{subtitles:crate::subtitles::Options::default(),transcode:crate::transcode::Options::default(),output_dir:get_default_download_dir(),proxy:"http://127.0.0.1:7897".into(),use_proxy:true,cookies_browser:String::new(),quality_height:None,output_mode:"original".into(),quality_mode:"balanced".into(),keep_source:true,concurrency:2,auto_update:true,batch_output_dir:String::new(),filename_suffix:false,filename_codecs:false}}}
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Task {#[serde(default)] pub subtitle_pending:bool,pub id:String,pub kind:String,pub source:String,pub title:String,pub status:String,pub stage:String,pub percent:f64,pub speed:String,pub eta:String,pub error:String,pub created:u64,pub settings:Settings,pub metadata:Option<ProbeResult>,pub video_format_id:Option<String>,pub audio_format_id:Option<String>,pub output:Option<DownloadResult>,pub downloaded:Option<DownloadResult>,#[serde(default)] pub name_conflict:Option<engine::NameConflict>,#[serde(default)] pub conflict_action:String}
...
if let Some(t)=data.tasks.iter_mut().find(|t|t.id==task.id){
    match result {
        Ok(output)=>{t.subtitle_pending=false;t.name_conflict=None;t.output=Some(output);t.status="done".into();t.stage="已完成".into();t.percent=100.;t.error.clear()},
        Err(e)=>{
            if flag.load(Ordering::Relaxed) {
                if q.closing.load(Ordering::Relaxed){t.status="interrupted".into();t.stage="待恢复".into()}
                    ...
                    t.stage=if t.subtitle_pending{"已暂停，继续后重新获取字幕"}else if t.kind=="transcode"||t.downloaded.is_some(){"已暂停，继续后重新转码"}else{"已暂停"}.into();
            ...
            } else {t.status="error".into();t.stage=if t.subtitle_pending{"视频已保存，字幕失败"}else if t.downloaded.is_some(){"下载成功，转码失败"}else{"失败"}.into();t.error=e}
        }
    }
...
fn execute(app:&AppHandle,mut task:Task)->Result<DownloadResult,String>{
    ...
    if task.settings.output_mode=="subtitles" {return crate::subtitles::download(app,&task.source,&task.settings,None)}
    if let Some(downloaded)=task.downloaded.clone(){let output=if task.subtitle_pending{downloaded}else{transcode(app,&task,&downloaded.output_path)?};return finish_subtitles(app,&task,output)}
    ...
    let output=if compatible {edit(app,&task.id,|t|t.downloaded=Some(output.clone()),true);transcode(app,&task,&output.output_path)?}else{output};
    finish_subtitles(app,&task,output)
}
fn finish_subtitles(app:&AppHandle,task:&Task,mut output:DownloadResult)->Result<DownloadResult,String>{
    if task.settings.subtitles.enabled {
        edit(app,&task.id,|t|{t.downloaded=Some(output.clone());t.subtitle_pending=true},true);
        let subtitles=crate::subtitles::download(app,&task.source,&task.settings,Some(Path::new(&output.output_path)))?;
        output.skipped=false;output.log.push_str("\n");output.log.push_str(&subtitles.log);
    }Ok(output)
}
