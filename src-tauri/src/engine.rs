...
let receipt=records.join(format!("{key}.json"));if let Ok(bytes)=fs::read(&receipt).or_else(|_|fs::read(records.join("legacy").join(format!("{key}.json")))).or_else(|_|fs::read(dir.join(".video-downloader").join(format!("{key}.json")))){if let Ok(mut output)=serde_json::from_slice::<DownloadResult>(&bytes){if let Ok(stats)=probe_media_stats(&app,Path::new(&output.output_path)){if stats.size_bytes>0{output.skipped=true;output.log="同一视频及格式的有效输出已存在，已跳过".into();return Ok(output)}}}}
